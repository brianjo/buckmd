...
An `android_manifest()` rule is used to generate an [Android
Manifest](http://developer.android.com/guide/topics/manifest/manifest-intro.html)
to be used by [`android_binary`](/rule/android_binary.html) and
[`android_aar`](/rule/android_aar.html) rules. This rule takes a
skeleton manifest, and merges it with manifests found in any deps.
...
-   ::: {#skeleton}
    ### `skeleton`{.argName} [(required)]{.argDefault} [\#](#skeleton){.inline-link}

    Either a [build target](/concept/build_target.html) or a path to a
    file representing the manifest that will be merged with any
    manifests associated with this rule\'s `deps`.
    :::
...
-   ::: {#deps}
    ...
    The manifest files of the
    [`android_library`](/rule/android_library.html) rules will be
    filtered out to become dependent source files for the manifest.
    :::
...
You could also use a [`genrule`](/rule/genrule.html) to generate the
manifest file and reference the [build
target](/concept/build_target.html) in the `skeleton` argument.
:::
