...
directory which is located in the root of your [Buck
project](/about/overview.html). However, you should not try to manually
modify the contents of `buck-out`. Buck\'s implementation assumes that
...
-   `--dry-run`
    ...
    Print a list of the directories that would be removed if the command
    ran normally.

-   `--keep-cache`

    Keep the local (dir) caches. For more information about the local
    caches, see the [`[cache]`](/files-and-dirs/buckconfig.html#cache)
