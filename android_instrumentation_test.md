...
A `android_instrumentation_test()` rule is used to define apks that
should be used to run Android instrumentation tests.
...
-   ::: {#apk}
    ### `apk`{.argName} [(required)]{.argDefault} [\#](#apk){.inline-link}

    The APK containing the tests. Can be an
    [`android_binary`](/rule/android_binary.html), an
...
Here is an example of an `android_instrumentation_test()` rule that
tests an `android_binary()`.
