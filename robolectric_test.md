...
::: overview
A `robolectric_test()` rule is used to define a set of `.java` files
that contain tests to run via JUnit with Robolectric test runner. It
extends from `java_test()` rule.
...
-   ::: {#robolectric_runtime_dependency}
    ### `robolectric_runtime_dependency`{.argName} [(defaults to `None`)]{.argDefault} [\#](#robolectric_runtime_dependency){.inline-link}

    Robolectric only runs in offline mode with buck. Specify the
    relative directory containing all the jars Robolectric uses at
    runtime.
    :::

-   ::: {#robolectric_manifest}
    ### `robolectric_manifest`{.argName} [(defaults to `None`)]{.argDefault} [\#](#robolectric_manifest){.inline-link}

    An optional [Android
    Manifest](http://developer.android.com/guide/topics/manifest/manifest-intro.html)
    for the rule to declare any permissions or intents it may need or
    want to handle. May either be a file or a
    [`android_manifest`](/rule/android_manifest.html) target.
    :::

-   ::: {#force_final_resource_ids}
    ### `force_final_resource_ids`{.argName} [(defaults to `True`)]{.argDefault} [\#](#force_final_resource_ids){.inline-link}

    Set it to false if using Robolectric version greater than 3.2 or
    higher as Robolectric requires resources id\'s to be non final.
    :::
...
-   ::: {#use_binary_resources}
    ### `use_binary_resources`{.argName} [(defaults to `False`)]{.argDefault} [\#](#use_binary_resources){.inline-link}

    Tells Robolectric to use binary resources mode, which reads
    resources from a resource APK rather than the filesystem. This
    requires Robolectric 4+.
    :::
