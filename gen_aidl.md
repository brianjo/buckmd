...
::: overview
A `gen_aidl()` rule is used to generate `.java` files from `.aidl`
files.
...
-   ::: {#aidl}
    ### `aidl`{.argName} [(required)]{.argDefault} [\#](#aidl){.inline-link}

    The path to an `.aidl` file to convert to a `.java` file.
    :::

-   ::: {#import_path}
    ### `import_path`{.argName} [(required)]{.argDefault} [\#](#import_path){.inline-link}

    The search path for import statements for the aidl command. (This is
    the `-I` argument when invoking aidl from the command line. For many
    apps it will be the base dir where all aidl files are, with project
    root as its parent, e.g. `app/src/main/aidl`.). This is the same as
    the path to the `aidl` file relative to what would be returned from
    [`buck root`](/command/root.html).
    :::

-   ::: {#aidl_srcs}
    ### `aidl_srcs`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#aidl_srcs){.inline-link}
