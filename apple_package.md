...
An `apple_package()` rule takes the output of an [`apple_bundle()`
rule](/rule/apple_bundle.html) and compresses it in an IPA (iOS App
Store Package) file. This rule can be customized using the config
options
[`[apple].package_command`](/files-and-dirs/buckconfig.html#apple.package_command)
and
[`[apple].package_extension`](/files-and-dirs/buckconfig.html#apple.package_extension).
...
-   ::: {#bundle}
    ### `bundle`{.argName} [(required)]{.argDefault} [\#](#bundle){.inline-link}

    A [build target](/concept/build_target.html) identifying an
    [`apple_bundle()` rule](/rule/apple_bundle.html) whose output will
