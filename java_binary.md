...
::: overview
A `java_binary()` rule is used to create a JAR file of the compiled
.class files and resources of the `java_library()` rules on which it
depends.
...
-   ::: {#name}
    ...
    target](/concept/build_target.html). Also, the name of the generated
    JAR file.
    :::
...
-   ::: {#deps}
    ...
    Rules (normally of type `java_library`) that should be compiled and
    whose `.class` files and resources should be included in the
    generated JAR file.
    :::

-   ::: {#main_class}
    ### `main_class`{.argName} [(defaults to `None`)]{.argDefault} [\#](#main_class){.inline-link}

    If provided, this will be the value specified as the `Main-Class`
    attribute of the `META-INF/MANIFEST.MF` file in the generated JAR
    file. Also, when this rule is used as an executable in a
    [`genrule()`](genrule.html), `main_class` will indicate the class
    whose `main()` method will be invoked to process the command-line
    arguments. This is consistent with the expected usage of
    `java -jar<name.jar> <args>`.
    :::

-   ::: {#manifest_file}
    ### `manifest_file`{.argName} [(defaults to `None`)]{.argDefault} [\#](#manifest_file){.inline-link}

    If provided, this manifest will be used when generating the JAR
    file. If combined with `main_class`, the specified manifest file
    will be used but the `main_class` will override the main class in
    the manifest.
    :::

-   ::: {#meta_inf_directory}
    ### `meta_inf_directory`{.argName} [(defaults to `None`)]{.argDefault} [\#](#meta_inf_directory){.inline-link}

    Note: This has beta support currently. If provided, the contents in
    this directory will end up in the `META-INF` directory inside the
    generated JAR file.
    :::

-   ::: {#blacklist}
    ### `blacklist`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#blacklist){.inline-link}

    A list of patterns that identify files to exclude from the final
    generated JAR file. Example:
