...
A cxx_binary() rule builds a native executable from the supplied set of
C/C++ source files and dependencies. If C/C++ library dependencies are
listed, the generated native executable will request and link against
their static archives (which are \*not\* built using
[PIC](http://en.wikipedia.org/wiki/Position-independent_code)).
...
### `#strip-debug`{.argName} [(required)]{.argDefault} [\#](##strip-debug){.inline-link}

If appended to the target name, it will strip debugging symbols, but
saving local and global symbols.

### `#strip-non-global`{.argName} [(required)]{.argDefault} [\#](##strip-non-global){.inline-link}

If appended to the target name, it will strip non-global symbols, but
keeping external symbols. This is preferred for dynamic shared
libraries.

### `#strip-all`{.argName} [(required)]{.argDefault} [\#](##strip-all){.inline-link}

If appended to the target name, it will completely strip the binary,
removing the symbol table and relocation information. This is preferred
