...
[`genrule`](/rule/genrule.html) by using the `exe` macro:
...
-   ::: {#exe}
    ### `exe`{.argName} [(defaults to `None`)]{.argDefault} [\#](#exe){.inline-link}

    A [build target](/concept/build_target.html) for a rule that outputs
    an executable, such as an [`sh_binary`](/rule/sh_binary.html).
    :::

-   ::: {#platform_exe}
    ### `platform_exe`{.argName} [(defaults to `{}`)]{.argDefault} [\#](#platform_exe){.inline-link}

    A mapping from platforms to [build
    target](/concept/build_target.html). enables you to override `exe`
    per host platform.

    If present, `exe` will be used as a fallback on host platforms that
    are not specified in `platform_exe`.

    It is possible to omit `exe` when providing `platform_exe`. In that
    case, the build will fail if the command is invoked on a platform
    not specified in the mapping.

    Valid platforms are all values of the [`Platform`
...
-   ::: {#args}
    ...
    `exe` at startup. These arguments support a subset of Buck\'s
    [string parameter macros](/function/string_parameter_macros.html).
    Only the `$(location ...)` and `$(exe ...)` macros are supported
    currently.
    :::
...
-   ::: {#env}
    ...
    A map of environment variables that will be passed to the executable
    represented by `exe` on startup. Environment variables support the
    same macros as arguments.
    :::
