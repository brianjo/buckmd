...
A cgo_library() rule builds an object from the supplied set of Go/C
source files and dependencies. The outputs are linked into go executable
in the last step (compile). The \'go build\' command would collect the
cgo directives from the source files, however with buck the flags needs
to be passed in the cgo_library manually This rule borrows from
[`cxx_binary`](/rule/cxx_binary.html) since C/C++ sources are being
compiled.
...
-   ::: {#package_name}
    ### `package_name`{.argName} [(defaults to `go.prefix + path relative to the buck root`)]{.argDefault} [\#](#package_name){.inline-link}

    Sets the full name of the package being compiled. This defaults to
    the path from the buck root. (e.g. given a ./.buckconfig, a rule in
    ./a/b/BUCK defaults to package \"a/b\")
    :::
...
-   ::: {#srcs}
    ...
    be compiled with the CGO compiler. Each file needs to have
    `import "C"` declared.
    :::

-   ::: {#go_srcs}
    ### `go_srcs`{.argName} [(required)]{.argDefault} [\#](#go_srcs){.inline-link}

    The set of source files to be compiled by this rule. Go (`.go`)
    files are compiled with the Go compiler. In contrast to the `srcs`
    argument, these files *cannot* have `import "C"` declared.
    :::
...
-   ::: {#cgo_compiler_flags_arg}
    ### `cgo_compiler_flags_arg`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#cgo_compiler_flags_arg){.inline-link}
...
-   ::: {#go_compiler_flags}
    ### `go_compiler_flags`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#go_compiler_flags){.inline-link}

    The set of additional compiler flags to pass to `go tool compile`.
    :::

-   ::: {#go_assembler_flags}
    ### `go_assembler_flags`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#go_assembler_flags){.inline-link}
