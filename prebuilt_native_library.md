...
-   ::: {#native_libs}
    ### `native_libs`{.argName} [(defaults to `None`)]{.argDefault} [\#](#native_libs){.inline-link}

    Path to a directory containing native libraries. This directory has
    subdirectories for different architectures, such as `armeabi` and
    `armeabi-v7a`.
    :::
...
Most of the time, a `prebuilt_native_library` is private to the
[`android_library`](/rule/android_library.html) that uses it:
