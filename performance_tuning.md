...
Buck [does a lot of work](/concept/what_makes_buck_so_fast.html) to make
...
![Chrome Tracing
Sample](/static/buck_chrome_sample.png){#chrome_sample}The Chrome team
has built an awesome framework for viewing performance traces right
inside of
...
Consult the trace viewer\'s [project
...
`buck-out/log/traces/`. Buck will save a file in the format
`build.[timestamp].trace`, and then create a symlink from the most
