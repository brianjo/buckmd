...
-   ::: {#deps}
    ...
    A list of dependencies of this bundle as build targets. You can
    embed application extensions by specifying the extension\'s bundle
    target. To include a WatchKit app, append the flavor `#watch` to the
    target specification. Buck will automatically substitute the
    appropriate platform flavor (either `watchsimulator` or `watchos`)
    based on the parent.
    :::

-   ::: {#product_name}
    ### `product_name`{.argName} [(required)]{.argDefault} [\#](#product_name){.inline-link}

    The name of the resulting bundle and binary. The setting behaves
    like PRODUCT_NAME Xcode build setting. For example, if your rule is
    named \"MyApp\" and extension is \"app\", by default buck will
    generate MyApp.app bundle. But if you will set product name to
    \"SuperApp\", bundle will get \"SuperApp.app\" name.
    :::

-   ::: {#extension}
    ### `extension`{.argName} [(required)]{.argDefault} [\#](#extension){.inline-link}

    The extension of the generated bundle. For example `'app'` for an
    application bundle or `'appex'` for an application extension bundle.
    :::

-   ::: {#binary}
    ### `binary`{.argName} [(required)]{.argDefault} [\#](#binary){.inline-link}

    A [build target](/concept/build_target.html) identifying an
    [`apple_binary()` rule](/rule/apple_binary.html) or an
    [`apple_library()` rule](/rule/apple_library.html) whose output will
    be used as the main executable binary of the generated bundle. The
    required rule type depends on the value in the `extension`
    attribute. For example, application bundles expect a binary (e.g.
    `'//Apps/MyApp:MyApp'`), application extension bundles expect a
    shared library (e.g. `'//Libraries/MyLibrary:MyLibrary#shared'`).
    :::
...
-   ::: {#asset_catalogs_compilation_options}
    ### `asset_catalogs_compilation_options`{.argName} [(required)]{.argDefault} [\#](#asset_catalogs_compilation_options){.inline-link}

    A dict holding parameters for asset catalogs compiler (actool). Its
    options include:
...
-   ::: {#ibtool_flags}
    ### `ibtool_flags`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#ibtool_flags){.inline-link}

    List of flags to be passed to ibtool during interface builder file
    compilation.
    :::
