...
A `prebuilt_haskell_library()` rule is used to identify Haskell prebuilt
libraries and their associated interface files.
...
-   ::: {#staticInterfaces}
    ### `staticInterfaces`{.argName} [(defaults to `None`)]{.argDefault} [\#](#staticInterfaces){.inline-link}

    Directory of interface files dependents will use when building for a
    statically linked top-level target.
    :::

-   ::: {#sharedInterfaces}
    ### `sharedInterfaces`{.argName} [(defaults to `None`)]{.argDefault} [\#](#sharedInterfaces){.inline-link}
...
-   ::: {#static_libs}
    ...
    The libraries to use when building a statically linked top-level
    target.
    :::
...
-   ::: {#shared_libs}
    ...
    A map of shared library names to shared library paths to use when
    building a dynamically linked top-level target.
    :::

-   ::: {#exported_compiler_flags}
    ### `exported_compiler_flags`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#exported_compiler_flags){.inline-link}

    Compiler flags used by dependent rules when compiling with this
    library.
    :::
...
-   ::: {#cxxHeaderDirs}
    ### `cxxHeaderDirs`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#cxxHeaderDirs){.inline-link}

    Directories with C/C++ headers which can be included by dependent
    C/C++ rules.
    :::
