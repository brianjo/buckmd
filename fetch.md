...
::: overview
A command that fetches one or more resources defined by
[`remote_file`](/rule/remote_file.html) rules. This should generally be
run before [`buck build`](/command/build.html) if
[`remote_file`](/rule/remote_file.html) rules are used in the
repository. You must specify one or more [build
target](/concept/build_target.html)s on the command line.
...
-   `--verbose` `(-v)` How verbose logging to the console should be,
    with 1 as the minimum and 10 as the most verbose.
:::
