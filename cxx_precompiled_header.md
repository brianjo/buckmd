...
[`cxx_library`](/rule/cxx_library.html) or a
[`cxx_binary`](/rule/cxx_binary.html).

This header file is precompiled by the preprocessor on behalf of the C,
C++, Objective-C, or Objective-C++ rule using it, via its
`precompiled_header` parameter. Afterwards the precompiled header is
...
-   ::: {#src}
    ### `src`{.argName} [(required)]{.argDefault} [\#](#src){.inline-link}

    The path to the header file that should be precompiled. Only one
    header file can be specified. But of course this header could
    include any number of other headers. The included headers could
    belong to \-- that is, be `exported_headers` from \-- another rule,
    in which case, the rule would have to be added to `deps` as usual.
    :::

-   ::: {#deps}
    ### `deps`{.argName} [(defaults to `None`)]{.argDefault} [\#](#deps){.inline-link}

    Dependency rules which export headers used by the header specified
    in `src`.
    :::
...
example we indicate that `common.h` is to be precompiled when used by
another build rule.
...
compiling `main.cpp`, and the resulting PCH will be used in `main`\'s
compilation.
...
list of build flags will change; not just to employ PCH with e.g.
`-include-pch` (if using Clang), but also, to alter the sequence of
