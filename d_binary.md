...
A d_binary() rule builds a native executable from the supplied set of D
...
-   ::: {#srcs}
    ...
    The set of D source files to be compiled by this rule. Each element
    should be a string specifying a source file (e.g. `'foo/bar.d'`).
    :::

-   ::: {#deps}
    ### `deps`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#deps){.inline-link}

    The set of dependencies of this rule. Each element should be a
    string specifying a d_library rule defined elsewhere (e.g. `':foo'`
    or \'//foo:bar\').
    :::

-   ::: {#linker_flags}
    ### `linker_flags`{.argName} [(required)]{.argDefault} [\#](#linker_flags){.inline-link}

    The list of flags to be passed to the linker. Each element should be
    a string specifying a linker flag (e.g. `'--as-needed'`).
    :::
