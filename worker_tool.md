...
use the `worker_tool()` rule in conjunction with
[`genrule`](/rule/genrule.html). Buck then starts the external tool once
and reuses it by communicating with it over `stdin` and `stdout` using a
...
-   ::: {#exe}
    ### `exe`{.argName} [(required)]{.argDefault} [\#](#exe){.inline-link}

    A [build target](/concept/build_target.html) for a rule that outputs
    an executable, such as an [`sh_binary`](/rule/sh_binary.html). Buck
    runs this executable only once per build.
    :::

-   ::: {#args}
    ### `args`{.argName} [(defaults to `None`)]{.argDefault} [\#](#args){.inline-link}

    A string of args that is passed to the executable represented by
    `exe` on initial startup.
    :::

-   ::: {#max_workers}
    ### `max_workers`{.argName} [(defaults to `1`)]{.argDefault} [\#](#max_workers){.inline-link}

    The maximum number of workers of this type that Buck starts. Use
    `-1` to allow the creation of as many workers as necessary.
    :::

-   ::: {#max_workers_per_thread_percent}
    ### `max_workers_per_thread_percent`{.argName} [(defaults to `None`)]{.argDefault} [\#](#max_workers_per_thread_percent){.inline-link}
    ...
    greater than or equal to `1` and less than or equal to `100`. Only
    one of `max_workers` and `max_workers_per_thread_percent` may be
    specified.
    :::

-   ::: {#env}
    ### `env`{.argName} [(defaults to `None`)]{.argDefault} [\#](#env){.inline-link}

    A map of environment variables that is passed to the executable
    represented by `exe` on initial startup.
    :::

-   ::: {#persistent}
    ### `persistent`{.argName} [(defaults to `False`)]{.argDefault} [\#](#persistent){.inline-link}

    If set to true, Buck does not restart the tool unless the tool
    itself changes. This means the tool persists across multiple Buck
    commands without being shut down and may see the same rule being
    built more than once. Be careful not to use this setting with tools
    that don\'t expect to process the same input---with different
    contents---twice!
    :::
...
Consider the following [build rules](/concept/build_rule.html):
