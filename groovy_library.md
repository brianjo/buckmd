...
A `groovy_library()` rule is used to define a set of Groovy files that
can be compiled together. It can also be used to cross compile a set of
Groovy and Java files. The main output of a `groovy_library()` rule is a
single JAR file containing all of the compiled class files and
resources.
...
-   ::: {#srcs}
    ...
    The set of files to compile for this rule. Usually these will all
    end in `.groovy`, but if any of the files end in`.java`, cross
    compilation using the jdk found in `JAVA_HOME` will occur.
    :::

-   ::: {#resources}
    ### `resources`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#resources){.inline-link}

    This is the same as in [`java_library`](/rule/java_library.html).
    :::

-   ::: {#deps}
    ### `deps`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#deps){.inline-link}

    Rules (usually other `groovy_library` or `java_library` rules) that
    are used to generate the classpath required to compile this
    `groovy_library`. This is the same as in
...
-   ::: {#exported_deps}
    ...
    Other `groovy_library` and `java_library` rules that depend on this
    rule will also include its `exported_deps` in their classpaths. This
    is the same as in [`java_library`](/rule/java_library.html).
    :::
...
-   ::: {#extra_groovyc_arguments}
    ### `extra_groovyc_arguments`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#extra_groovyc_arguments){.inline-link}
