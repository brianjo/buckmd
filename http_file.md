...
An `http_file()` rule is used to download files from the Internet to be
used as dependencies for other rules. This rule only downloads single
files, and can optionally make them executable (see
[`executable`](/rule/http_file.html#executable)) These rules are
downloaded by running [`buck fetch`](/command/fetch.html), or can be
downloaded as part of [`buck build`](/command/build.html) by setting
[`[download].in_build`](/files-and-dirs/buckconfig.html#download.in_build)
...
-   ::: {#name}
    ...
    target](/concept/build_target.html). This is also used for the name
    of the file on disk if  [`out`](/rule/http_file.html#out)  is not
    provided.
    :::

-   ::: {#urls}
    ### `urls`{.argName} [(required)]{.argDefault} [\#](#urls){.inline-link}

    A list of urls to attempt to download from. They are tried in order,
    and subsequent ones are only tried if the download fails. If
    validation fails, a new URL is not used. Supported protocols are
    \"http\", \"https\", and \"mvn\".
    :::

-   ::: {#sha256}
    ### `sha256`{.argName} [(required)]{.argDefault} [\#](#sha256){.inline-link}

    The [`SHA-256`](//wikipedia.org/wiki/SHA-2) hash of the downloaded
    artifact. Buck verifies this is correct and fails the fetch command
    if it doesn\'t match in order to guarantee repeatable builds.
    :::

-   ::: {#out}
    ### `out`{.argName} [(defaults to `None`)]{.argDefault} [\#](#out){.inline-link}

    An optional name to call the downloaded artifact. Buck will generate
    a default name if one is not provided that uses the `name` of the
    rule.
    :::

-   ::: {#executable}
    ### `executable`{.argName} [(required)]{.argDefault} [\#](#executable){.inline-link}

    Whether or not the file should be made executable after downloading.
    If true, this can also be used via [`buck run`](/command/run.html)
    and the `$(exe )` [string parameter
...
Tooling can also be fetched with `http_file()` and used by a
