...
::: overview
Buck has restrictions on importing arbitrary Python modules in [build
file](/concept/build_file.html)s which make it harder to accidentally
violate assumptions about determinism of [build
file](/concept/build_file.html)s and build rules. The
`allow_unsafe_import()` function may be used to create a context that
lifts the restrictions on module importing.
...
[`build_file_import_whitelist`](/concept/buckconfig.html#project.build_file_import_whitelist)
config setting.
...
Buck has no way to know if the results of uncontrolled file system or
network access change, and will not reevaluate the build file if that
happens.
