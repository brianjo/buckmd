...
Buck](/setup/install.html), the manual build process to get the repo.
...
Once you have built the plugin, you can install it by following the
...
1.  Generate the grammar and the lexer with the Grammar-Kit plugin.
    -   You can install the grammar kit plugin from
        `third-party/java/grammar-kit/grammar-kit.jar` by following the
        ...
        files by navigating in Intellij to
...
2.  [Configure the Intellij Platform Plugin
    ...
    and set it as the [SDK of ideabuck
...
3.  Remove the Buck Intellij Plugin, `tools/ideabuck` from [the Compiler
    Excludes
...
5.  Run/Debug the plugin or create a new release of it by going
    to` Build > Prepare all plugin modules for deployment`.
:::
