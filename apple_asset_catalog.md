...
An `apple_asset_catalog()` rule contains resources stored in Apple asset
catalog directories. This rule does not have any output on its own and
...
and placed into the final output bundle together.
...
-   ::: {#dirs}
    ### `dirs`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#dirs){.inline-link}

    Set of paths of Apple asset catalogs contained by this rule. All
    paths have to end with the `.xcassets` extension and be compatible
    with the asset catalog format used by Xcode.
    :::

-   ::: {#app_icon}
    ### `app_icon`{.argName} [(defaults to `None`)]{.argDefault} [\#](#app_icon){.inline-link}

    An optional reference to a `.appiconset` containing a image set
    representing an application icon. (The extension itself should not
    be included.) This parameter may be specified at most once in a
    given `apple_bundle`\'s transitive dependencies.
    :::

-   ::: {#launch_image}
    ### `launch_image`{.argName} [(defaults to `None`)]{.argDefault} [\#](#launch_image){.inline-link}

    An optional reference to a `.launchimage` containing a image set
    representing an application launch image. (The extension itself
    should not be included.) This parameter may be specified at most
    once in a given `apple_bundle`\'s transitive dependencies.
    :::
