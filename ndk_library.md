...
::: overview
An `ndk_library()` is used to define a set of C/C++ files, an
`Android.mk` and an `Application.mk` file that are used by the NDK\'s
`ndk-build` tool to generate one or more shared objects.
...
-   ::: {#srcs}
    ### `srcs`{.argName} [(defaults to `[**/*.{cpp,c,cc,cxx,h,hpp,mk}]`)]{.argDefault} [\#](#srcs){.inline-link}

    The set of files to compile for this rule. If not provided, `buck`
    assumes that all files with the following extensions are part of the
    build: `c, cpp, cc, cxx, h, hpp, mk`.
    :::
...
-   ::: {#flags}
    ### `flags`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#flags){.inline-link}

    Array of strings passed verbatim to `ndk-build`. Normally this is
    not needed, but in some cases you may want to put something here.
    For example, this can be used to build the libraries in debug mode
    (`NDK_DEBUG=1`) or set the number of jobs spawned by `ndk-build` (by
    default, the same as the number of cores).
    :::

-   ::: {#is_asset}
    ### `is_asset`{.argName} [(defaults to `False`)]{.argDefault} [\#](#is_asset){.inline-link}

    Normally native shared objects end up in a directory in the root of
    the APK named `lib/`. If this parameter is set to `True`, then these
    objects are placed in `assets/lib/`. Placing shared objects in a
...
Unlike the default invocation of `ndk-build`, `buck` will put all
