...
::: overview
A `prebuilt_apple_framework()` rule represents a set of Objective-C/C++
source files and is very similar to a
[`prebuilt_cxx_library()`](/rule/prebuilt_cxx_library.html) rule.

## Arguments

-   ::: {#preferred_linkage}
    ### `preferred_linkage`{.argName} [(required)]{.argDefault} [\#](#preferred_linkage){.inline-link}

    How to link to a binary: use `dynamic` for a dynamic framework, and
    `static` for old universal static frameworks manually lipo-ed
    together. `dynamic` will copy the frameworks into the `Frameworks`
    directory of an Apple bundle, and configure framework search paths
    and linker flags.`static` will copy the resources of the framework
