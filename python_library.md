...
A `python_library()` rule is used to group together Python source files
and resources to package them into a PEX using a top-level
[`python_binary`](/rule/python_binary.html) rule.
...
-   ::: {#deps}
    ...
    Other `python_library()` rules that list `srcs` from which this rule
    imports modules.
    :::
