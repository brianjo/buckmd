...
A `remote_file()` rule is used to download files from the Internet to be
used as dependencies for other rules. These rules are downloaded by
running [`buck fetch`](/command/fetch.html), or can be downloaded as
part of [`buck build`](/command/build.html). See the note there about
the `.buckconfig` setting to configure that.
...
-   ::: {#url}
    ### `url`{.argName} [(required)]{.argDefault} [\#](#url){.inline-link}

    You can specify an `http`, `https`, or a `mvn` URL. If you specify a
    `mvn` URL, it will be decoded as described in the [javadocs for
    MavenUrlDecoder](/javadoc/com/facebook/buck/file/MavenUrlDecoder.html).
    See the example section below.
    :::

-   ::: {#sha1}
    ### `sha1`{.argName} [(required)]{.argDefault} [\#](#sha1){.inline-link}

    The [`SHA-1`](//wikipedia.org/wiki/SHA-1) hash of the downloaded
...
-   ::: {#type}
    ### `type`{.argName} [(defaults to `data`)]{.argDefault} [\#](#type){.inline-link}
...
referenced by a [`prebuilt_jar`](/rule/prebuilt_jar.html).
