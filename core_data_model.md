...
An `core_data_model()` rule contains models for Apple\'s Core Data
framework. This rule does not have any output on its own and can be
built only as a dependency (either direct or transitive) of an
[`apple_bundle()` rule](/rule/apple_bundle.html), in which case all
`core_data_model()` rules that the bundle rule depends on are merged and
placed into the final output bundle together.
