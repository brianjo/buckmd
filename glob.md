...
-   The first parameter is unnamed and is a list of patterns that match
    files under the current directory.
-   `exclude` (defaults to `[]`) A list of patterns that identify files
    that should be removed from the set specified by the first
    parameter. You cannot specify both the `exclude` parameter and the
    `excludes` parameter because they serve the same purpose.
-   `include_dotfiles` (defaults to `False`) Specifies whether `*` and
    `**` patterns should capture file and directory names that start
    with a dot (`.`). By default, this parameter is false, which
    indicates that files that start with a dot are automatically
    excluded.
-   `excludes` (defaults to `[]`) DEPRECATED\
    This parameter is deprecated; use the `exclude` parameter instead of
    `excludes`.\
    A list of patterns that identify files that should be removed from
