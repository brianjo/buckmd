...
::: overview
An `apk_genrule()` rule is used to post-process an APK. What separates
an apk_genrule from a genrule is apk_genrules are known by BUCK to
produce APKs, so commands like `buck install` or `buck uninstall` still
work. Additionally, `apk_genrule()` rules can be inputs to other
`apk_genrule()` rules.
...
-   ::: {#out}
    ...
    This argument only exists for historical reasons and it does not
...
Here is an example of a couple `apk_genrule()` open up an APK, do some
super signing, and then zipalign that APK again.
