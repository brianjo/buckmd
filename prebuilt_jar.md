...
::: overview
A `prebuilt_jar()` rule is used to identify a JAR file that is checked
into our repository as a precompiled binary rather than one that is
built from source by Buck. Frequently, these are used to reference
third-party JAR files (such as junit.jar) and are used as dependencies
of [`java_library()`](java_library.html) rules.
...
-   ::: {#binary_jar}
    ### `binary_jar`{.argName} [(required)]{.argDefault} [\#](#binary_jar){.inline-link}
...
-   ::: {#source_jar}
    ...
    `.class` in the `binary_jar`. This is frequently provided for
    debugging purposes.
    :::
...
-   ::: {#deps}
    ...
    Rules that must be built before this rule. Because the `binary_jar`
    is already built, there should be nothing to build, so this should
    be empty.
    :::
