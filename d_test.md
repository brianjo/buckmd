...
A `d_test()` rule is used to define a set of D source files that contain
tests to run via D\'s unittest support. The source code of the test must
...
-   ::: {#labels}
    ...
    `d_test()` rule when executing [`buck test`](/command/test.html).
    :::
