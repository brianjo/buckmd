...
::: overview
A `java_test()` rule is used to define a set of `.java` files that
contain tests to run via JUnit.
...
-   ::: {#srcs}
    ...
    Like [`java_library()`](java_library.html), all of the `.java` files
    specified by the `srcs` argument will be compiled when this rule is
    built. In addition, all of the corresponding `.class` files that are
    built by this rule will be passed as arguments to JUnit when this
    rule is run as a test. `.class` files that are passed to JUnit that
    do not have any methods annotated with `@Test` are considered failed
    tests, so make sure that only test case classes are specified as
    `srcs`. This is frequently done by specifying `srcs` as
    `glob(['**/*Test.java'])`.
    :::
...
-   ::: {#source}
    ### `source`{.argName} [(defaults to `'6'`)]{.argDefault} [\#](#source){.inline-link}
...
-   ::: {#target}
    ### `target`{.argName} [(defaults to `'6'`)]{.argDefault} [\#](#target){.inline-link}
...
-   ::: {#deps}
    ...
    Same as [`java_library()`](java_library.html). Must include JUnit
    (version 4.7 or later) as a dependency for JUnit tests. Must include
    TestNG (version 6.2 or later) and hamcrest as a dependencies for
    TestNG tests.
    :::

-   ::: {#test_type}
    ### `test_type`{.argName} [(defaults to `junit`)]{.argDefault} [\#](#test_type){.inline-link}

    Specifies which test framework to use. The currently supported
    options are \'junit\' and \'testng\'.
    :::
...
-   ::: {#fork_mode}
    ### `fork_mode`{.argName} [(defaults to `None`)]{.argDefault} [\#](#fork_mode){.inline-link}

    Controls whether tests will all be run in the same process or a
    process will be started for each set of tests in a class. (This is
    mainly useful when porting Java tests to Buck from Apache Ant which
    allows JUnit tasks to set a `fork="yes"` property. It should not be
    used for new tests since it encourages tests to not cleanup after
    themselves and increases the tests\' computational resources and
    running time.)
    ...
    `per_test`
    :   A process will be started for each test class in which all tests
        of that test class will run.
    :::
...
-   ::: {#std_out_log_level}
    ### `std_out_log_level`{.argName} [(defaults to `FINE`)]{.argDefault} [\#](#std_out_log_level){.inline-link}

    Log level for messages from the source under test that buck will
    output to std out.
...
-   ::: {#std_err_log_level}
    ### `std_err_log_level`{.argName} [(defaults to `WARNING`)]{.argDefault} [\#](#std_err_log_level){.inline-link}

    Same as `std_out_log_level`, but for std err.
    :::

-   ::: {#use_cxx_libraries}
    ### `use_cxx_libraries`{.argName} [(defaults to `False`)]{.argDefault} [\#](#use_cxx_libraries){.inline-link}

    Whether or not to build and link against
    [`cxx_library`](/rule/cxx_library.html) dependencies when testing.
    :::

-   ::: {#cxx_library_whitelist}
    ### `cxx_library_whitelist`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#cxx_library_whitelist){.inline-link}

    EXPERIMENTAL. List of cxx_libraries to build, if use_cxx_libraries
    is true. This can be useful if some dependencies are Android-only
    and won\'t build on the default platform.
    :::
...
-   ::: {#vm_args}
    ### `vm_args`{.argName} [(defaults to `[]`)]{.argDefault} [\#](#vm_args){.inline-link}
